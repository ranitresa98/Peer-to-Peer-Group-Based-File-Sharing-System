      ...
      ch= cin.get();
         ...
         if(ch=='A') //up
        {
         ...
         { 
          ...
          if(cursor_position<(rows-2) && cursor_position<dir_list.size() /*&& file_end==dir_list.size()*/ )
         { 
...
if(command_line_print==0)
  {moveTo(rows,1);cout<<"\x1b[K"; cout<<":"<<command;} 
...
size_t found = from_path.find_last_of("/");
  string parent=from_path.substr(0,found);
string to_path=parent+"/"+commands_with_args[2];
